...
class TestGoogleSubscriptionExchange(TestCase):
    ...
    def test_google_subscription_exchange(self):
        """[谷歌订阅]创建订阅订单，正常请求"""
        path = "/subscription/google-exchange"
        body = {
            "appid": get_app_id(),
            "device": self.device,
            "extend": self.extend,
        ...
        self.assertIsNotNone(response.json().get('result').get('google_pay_key'))
    ...
    def test_google_subscription_exchange_with_empty_body(self):
        """[谷歌订阅]创建订阅订单，参数校验：空表单"""
        path = "/subscription/google-exchange"
        body = {
        }
        response = sdk_http_client.post(path, body)
    ...
    def test_google_subscription_exchange_with_blank_body(self):
        """[谷歌订阅]创建订阅订单，参数校验：参数为空"""
        path = "/subscription/google-exchange"
        body = {
            "appid": "",
            "device": "",
    ...
    def test_google_subscription_exchange_with_null_body(self):
        """[谷歌订阅]创建订阅订单，参数校验：参数为null"""
        path = "/subscription/google-exchange"
        body = {
            "appid": None,
            "device": None,
    ...
    def test_google_subscription_exchange_without_role_id(self):
    ...
    def test_google_subscription_exchange_without_product_id(self):
    ...
    def test_google_subscription_exchange_without_price(self):
    ...
    def test_google_subscription_exchange_with_invalid_price(self):
    ...
    def test_google_subscription_exchange_with_invalid_price2(self):
    ...
    def test_google_subscription_exchange_with_invalid_price3(self):
    ...
    def test_google_subscription_exchange_with_invalid_price4(self):
    ...
    def test_google_subscription_exchange_with_invalid_price5(self):
...
class TestGoogleSubscriptionVerify(TestCase):
    def setUp(self) -> None:
        # 沙箱环境
        self.purchaseToken = "ekkapnaicknoimfaombfgbag.AO-J1OzOfT-j-NrTJByJnsvV8oXsuMvi1y6BY0RBK0AiPyxS_0fIpPeJQqCu4a0fI4rhtslr_9e4ruk-lN2gH6rJb3FaGtbLjg"
        self.purchase_data = "{\"orderId\":\"GPA.3391-0117-2444-78344\",\"packageName\":\"com.topjoy.sdk_demo\",\"productId\":\"com.topjoy.sdk8\",\"purchaseTime\":1692356993663,\"purchaseState\":0,\"purchaseToken\":\"ekkapnaicknoimfaombfgbag.AO-J1OzOfT-j-NrTJByJnsvV8oXsuMvi1y6BY0RBK0AiPyxS_0fIpPeJQqCu4a0fI4rhtslr_9e4ruk-lN2gH6rJb3FaGtbLjg\",\"obfuscatedAccountId\":\"as_20230818190943_VsdN6d\",\"quantity\":1,\"autoRenewing\":true,\"acknowledged\":false,\"developerPayload\":\"as_20230818190943_VsdN6d\"}"
        self.purchase_sign = ""
        self.developerPayload = "as_20230818190943_VsdN6d"
        # 测试环境
    ...
    def test_google_subscription_verify(self):
        """[Google订阅确认]已确认订单，重复确认"""
        path = "/subscription/google-verify"
        body = {
            "appid": get_app_id(),
        ...
        response = sdk_http_client.post(path, body)
        # {'error_no': 10009, 'message': '支付订单认证失败'}，token，data可能会过期，过期之后就会返回10009，过期之前返回0成功
        self.assertEqual(response.json().get('error_no'), 10009)

    def test_subscription_verify_with_empty_body(self):
        """[google订阅确认]参数校验，空表单"""
        path = "/subscription/google-verify"
        body = {
        }
    ...
    def test_subscription_verify_with_blank_body(self):
        """[google订阅确认]参数校验，参数为空"""
        path = "/subscription/google-verify"
        body = {
            "appid": "",
            "developerPayload": "",
    ...
    def test_subscription_verify_with_null_body(self):
        """[google订阅确认]参数校验，参数为null"""
        path = "/subscription/google-verify"
        body = {
            "appid": None,
